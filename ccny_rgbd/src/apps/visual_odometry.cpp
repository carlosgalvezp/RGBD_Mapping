...
void VisualOdometry::myCallBack(const CameraInfoMsg::ConstPtr& rgb_info_msg,
  ...
  ROS_INFO("[Visual Odometry] CallBack");
}
...
{
  // ****
  if (!nh_private_.getParam ("calibrate", calibrate_))
      calibrate_ = false;
...
void VisualOdometry::publishModelCloud()
{
  ...
  // write out features if calibrate
  if (calibrate_)
  {
      pcl::PCDWriter writer;
      const std::string path = cam_name_+"_features.pcd";
