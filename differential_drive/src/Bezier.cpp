...
int Bezier::compute_curve(float V_max,float A_max)	{
	...
	if(fabs(curve_length)<pow(V_max,2)/A_max)	{
			eta = sqrt(4*fabs(curve_length)/A_max);
	}
       	else	{
			eta = fabs(curve_length)/V_max + V_max/A_max;
		}
...
/**
 * Get the (dX,dY) values in the parameter u. 
 * @ float u : curve parameter
 */
...
/**
 * Get the (d2X,d2Y) values in the parameter u. 
 * @ float u : curve parameter
 */
