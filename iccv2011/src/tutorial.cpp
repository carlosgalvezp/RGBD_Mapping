...
class ICCVTutorial
{
  ...
  protected:
    ...
    void remove_InvalidKeypoints(typename pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_in,
                                 typename pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_out);

    void determineInitialTransformationSAC ();
...
//  detectKeypoints (target_segmented_, target_keypoints_);
    pcl::io::loadPCDFile ("/home/carlos/cam2_features.pcd", *source_keypoints_);
    pcl::io::loadPCDFile ("/home/carlos/cam1_features.pcd", *target_keypoints_);

    // Fill in the "I" component with a constant
    for(unsigned i=0;i< source_keypoints_->size();i++)
    {
      (source_keypoints_->at(i)).intensity = 1;
    }
    for(unsigned i=0;i< target_keypoints_->size();i++)
    {
      (target_keypoints_->at(i)).intensity = 1;
    }

  //Remove invalid points
  remove_InvalidKeypoints(source_keypoints_,source_keypoints_);
  remove_InvalidKeypoints(target_keypoints_,target_keypoints_);
...
//  determineInitialTransformationSAC ();
  determineInitialTransformation ();
  determineFinalTransformation ();

//  reconstructSurface ();

  Eigen::Matrix4f matrix = transformation_matrix_ * initial_transformation_matrix_;
...
template<typename FeatureType>
void ICCVTutorial<FeatureType>::remove_InvalidKeypoints(typename pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_in,
                                                        typename pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_out)
{
    std::cout<<"Removing invalid points from cloud of size: "<<cloud_in->size()<<"...";

    typename pcl::PointCloud<pcl::PointXYZI>::Ptr outTmp (new pcl::PointCloud<pcl::PointXYZI>);
    typename pcl::DefaultPointRepresentation<pcl::PointXYZI> point_representation;
...
template<typename FeatureType>
void ICCVTutorial<FeatureType>::determineInitialTransformationSAC ()
{
  cout << "initial alignment with RANSAC..." << std::flush;
  pcl::SampleConsensusInitialAlignment<pcl::PointXYZI, pcl::PointXYZI, FeatureType> sac;

  sac.setMinSampleDistance (0.05);
  sac.setMaxCorrespondenceDistance (0.1);
  sac.setMaximumIterations (5000);

  sac.setInputSource(source_keypoints_);
  sac.setSourceFeatures (source_features_);
  sac.setInputTarget (target_keypoints_);
  sac.setTargetFeatures (target_features_);

  typename pcl::registration::CorrespondenceEstimation<pcl::PointXYZI, pcl::PointXYZI>::Ptr cest
          (new pcl::registration::CorrespondenceEstimation<pcl::PointXYZI, pcl::PointXYZI>);
  sac.setCorrespondenceEstimation(cest);

  pcl::PointCloud<pcl::PointXYZI> tmp;
  sac.align(tmp);

  initial_transformation_matrix_ = sac.getFinalTransformation();
  pcl::transformPointCloud(*source_segmented_, *source_transformed_, initial_transformation_matrix_);
    ...
    switch (event.getKeyCode())
    {
      ...
      case '5':
        if (!visualizer_.removePointCloud("source_keypoints"))
        {
          ...
          visualizer_.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE,5,"source_keypoints");
        }
        break;
      ...
      case '6':
        if (!visualizer_.removePointCloud("target_keypoints"))
        {
          ...
          visualizer_.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE,5,"target_keypoints");
        }
        break;
...
//                  typename pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr source,
//                  typename pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr target,
//                  typename pcl::PointCloud<pcl::PointXYZI>::Ptr source_keypoints,
//                  typename pcl::PointCloud<pcl::PointXYZI>::Ptr target_keypoints);
...
//  {
//    pcl::copyPointCloud(*source_keypoints,*source_keypoints_);
//    pcl::copyPointCloud(*target_keypoints,*target_keypoints_);
//  }
//  else
//  {
//    detectKeypoints (source_segmented_, source_keypoints_);
...
//  if (argc < 6)
//  {
//    pcl::console::print_info ("Syntax is: %s <source-pcd-file> <target-pcd-file> <keypoint-method> <descriptor-type> <surface-reconstruction-method> <source-keypoints-pcd-file> <target-keypoints-pcd-file>\n", argv[0]);
//    pcl::console::print_info ("available <keypoint-methods>: 1 = Sift3D\n");
//    pcl::console::print_info ("                              2 = Harris3D\n");
//    pcl::console::print_info ("                              3 = Tomasi3D\n");
//    pcl::console::print_info ("                              4 = Noble3D\n");
...
//  pcl::PointCloud<pcl::PointXYZI>::Ptr source_kpts (new pcl::PointCloud<pcl::PointXYZI>);
//  pcl::PointCloud<pcl::PointXYZI>::Ptr target_kpts (new pcl::PointCloud<pcl::PointXYZI>);

//  if (argc > 6){
//      pcl::io::loadPCDFile (argv[6], *source_kpts);
//      pcl::io::loadPCDFile (argv[7], *target_kpts);

//      // Fill in the "I" component with a constant
//      for(unsigned i=0;i< source_kpts->size();i++)
//      {
//        (source_kpts->at(i)).intensity = 1;
//      }
//      for(unsigned i=0;i< target_kpts->size();i++)
//      {
//        (target_kpts->at(i)).intensity = 1;
//      }
//  }
...
//      feature_extractor->setSearchMethod (pcl::search::Search<pcl::PointXYZRGB>::Ptr (new pcl::search::KdTree<pcl::PointXYZRGB>));
//      feature_extractor->setRadiusSearch (0.05);
//      ICCVTutorial<pcl::FPFHSignature33> tutorial (keypoint_detector, feature_extractor, surface_reconstruction, source, target,source_kpts,target_kpts);
//      tutorial.run ();
//    }
//    break;
...
//      shot->setRadiusSearch (0.04);
//      pcl::Feature<pcl::PointXYZRGB, pcl::SHOT1344>::Ptr feature_extractor (shot);
//      ICCVTutorial<pcl::SHOT1344> tutorial (keypoint_detector, feature_extractor, surface_reconstruction, source, target,source_kpts,target_kpts);
//      tutorial.run ();
//    }
//    break;
...
//      pcl::Feature<pcl::PointXYZRGB, pcl::PFHSignature125>::Ptr feature_extractor (new pcl::PFHEstimation<pcl::PointXYZRGB, pcl::Normal, pcl::PFHSignature125>);
//      feature_extractor->setKSearch(50);
//      ICCVTutorial<pcl::PFHSignature125> tutorial (keypoint_detector, feature_extractor, surface_reconstruction, source, target,source_kpts,target_kpts);
//      tutorial.run ();
//    }
//    break;
...
//      pcl::Feature<pcl::PointXYZRGB, pcl::PFHRGBSignature250>::Ptr feature_extractor (new pcl::PFHRGBEstimation<pcl::PointXYZRGB, pcl::Normal, pcl::PFHRGBSignature250>);
//      feature_extractor->setKSearch(50);
//      ICCVTutorial<pcl::PFHRGBSignature250> tutorial (keypoint_detector, feature_extractor, surface_reconstruction, source, target,source_kpts,target_kpts);
//      tutorial.run ();
//    }
//    break;
